...
import { simpleCsvDataService, type ProjectDataSummary } from './services/simpleCsvDataService';
  ...
  const loadCsvData = async () => {
    ...
    try {
      ...
      if (p1Data.status === 'fulfilled') {
        console.log('项目1数据加载成功:', p1Data.value);
        setProject1Data(p1Data.value);
      } else {
        console.warn('项目1 样本数据加载失败:', p1Data.reason);
      }

      if (p2Data.status === 'fulfilled') {
        console.log('项目2数据加载成功:', p2Data.value);
        setProject2Data(p2Data.value);
      } else {
        console.warn('项目2 样本数据加载失败:', p2Data.reason);
      }

      message.success('样本数据加载完成');
    } catch (error) {
      console.error('样本数据加载失败:', error);
      message.error('样本数据加载失败');
    } finally {
      setCsvLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={16}>
          <Tabs defaultActiveKey="projects" type="card">
            ...
            <TabPane tab="全球分布" key="map">
              <Card className="h-96">
                <div className="flex items-center justify-center h-full text-gray-500">
                  <div className="text-center">
                    <GlobalOutlined className="text-4xl mb-4" />
                    <div>地图功能开发中</div>
                    <div className="text-sm mt-2">将显示全球项目分布</div>
                  </div>
                </div>
              </Card>
            </TabPane>
          ...
          >
            <div className="space-y-4">
              ...
              <Row gutter={[8, 8]}>
                <Col span={12}>
                  <div className="text-center p-3 bg-purple-50 rounded">
                    ...
                    <div className="text-sm text-gray-500">样本Bank数</div>
                  </div>
                </Col>
                <Col span={12}>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                ...
                {project1Data.length === 0 && project2Data.length === 0 && !csvLoading && (
                  <div className="text-center text-gray-400 py-4">
                    <div className="text-sm">暂无样本数据</div>
                    <div className="text-xs mt-1">请点击"刷新样本数据"加载</div>
                  </div>
                )}
              </div>
            </div>
      ...
      <Row gutter={[16, 16]} className="mt-6">
        <Col xs={24} md={12}>
          <Card title="系统状态" size="small">
            <Row gutter={[16, 8]}>
              <Col span={8}>
                <div className="text-center">
                  <div className="text-lg font-bold text-green-500">{summaryData.regions.Europe.projects}</div>
                  <div className="text-xs text-gray-500">正常项目</div>
                </div>
              </Col>
              <Col span={8}>
                <div className="text-center">
                  <div className="text-lg font-bold text-yellow-500">0</div>
                  <div className="text-xs text-gray-500">警告项目</div>
                </div>
              </Col>
              <Col span={8}>
                <div className="text-center">
                  <div className="text-lg font-bold text-red-500">0</div>
                  <div className="text-xs text-gray-500">异常项目</div>
                </div>
              </Col>
            </Row>
          </Card>
        ...
        <Col xs={24} md={12}>
          <Card title="今日数据" size="small">
            <Row gutter={[16, 8]}>
              <Col span={12}>
                <div className="text-gray-500 text-xs">充电量</div>
                <div className="font-medium">{summaryData.todayChargeEnergy} kWh</div>
              </Col>
              <Col span={12}>
                <div className="text-gray-500 text-xs">放电量</div>
                <div className="font-medium">{summaryData.todayDischargeEnergy} kWh</div>
              </Col>
            </Row>
          </Card>
        </Col>
