...
import { csvDataService, type ProjectDataSummary, type BankDisplayData } from '../../services/csvDataService';
import type { StandardBatteryData, BankTimeSeries } from '../../../csv-converter/src/types/index';

const { TabPane } = Tabs;

export const CsvDataPage: React.FC = () => {
  ...
  const [projectDetail, setProjectDetail] = useState<StandardBatteryData | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);

  // 模拟数据路径 - 实际使用时应该从配置或用户输入获取
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const [p1Data, p2Data] = await Promise.allSettled([
        csvDataService.loadProject1Data(PROJECT1_DATA_PATH),
        csvDataService.loadProject2Data(PROJECT2_DATA_PATH)
      ]);
  ...
  const loadProjectDetail = async (projectId: string) => {
    ...
    try {
      ...
      const detail = await csvDataService.getProjectDetail(projectId, dataPath);
   ...
   * 渲染Bank表格
   */
  const renderBankTable = (banks: BankTimeSeries[]) => {
    const bankDisplayData: BankDisplayData[] = banks.map(bank => 
      csvDataService.convertBankToDisplayData(bank)
    );
   ...
   * 计算平均SOC
   */
  const calculateAvgSoc = (banks: BankTimeSeries[]): number => {
   ...
   * 计算平均SOH
   */
  const calculateAvgSoh = (banks: BankTimeSeries[]): number => {
    if (banks.length === 0) return 0;
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex justify-between items-center">
        <div>
          <h2 className="text-xl font-semibold">CSV数据展示</h2>
        ...
        <div className="space-x-2">
          ...
          <Button 
            ...
            onClick={() => message.info('数据路径配置功能开发中')}
          >
            配置
          </Button>
        </div>
        ...
        <Col xs={12} sm={6}>
          <Card size="small" className="shadow-sm">
            <Statistic 
              ...
              suffix="%" 
              style={{ 
                color: [...project1Data, ...project2Data].length > 0 && 
                       Math.round([...project1Data, ...project2Data].reduce((sum, p) => {
                         const avgQuality = (p.dataQuality.completeness + p.dataQuality.accuracy + p.dataQuality.consistency + p.dataQuality.timeliness) / 4;
                         return sum + avgQuality;
                       }, 0) / [...project1Data, ...project2Data].length) >= 90 ? '#52c41a' : '#faad14'
              }}
            />
          </Card>
        </Col>
                      ...
                      <Col xs={24} md={8}>
                        <Card size="small" title="项目摘要">
                          <div className="space-y-2">
                            ...
                            <div className="flex justify-between">
                              <span>平均SOC</span>
                              <span>{calculateAvgSoc(projectDetail.banks).toFixed(1)}%</span>
                            </div>
                            <div className="flex justify-between">
                              <span>平均SOH</span>
                              <span>{calculateAvgSoh(projectDetail.banks).toFixed(1)}%</span>
                            </div>
                          </div>
                        </Card>
                      </Col>
